...
pub fn tween_factor<T: Send + Sync>(mut factor_q: Query<&mut TweenFactor<T>>, time: Res<Time>) {
    for mut factor in &mut factor_q {
        ...
        } else if !factor.timer.is_finished() {
...
pub fn tween_sprite_color_on_trigger<E: EntityEvent, B: Bundle>(
    color: impl Into<Color>,
) -> impl FnMut(On<E, B>, Commands) {
    let color = color.into();
    move |ev, mut cmd| {
        let card_e = ev.event_target();
...
>(
    color: impl Into<Color>,
) -> impl FnMut(On<E, B>, Commands, Query<&TSpriteRelationship>) {
    let color = color.into();
